...
//	var $input = $('<input type="hidden" name="photo[]">');
//	$input.val(a);
//    $('#photoForm').append($input);
    sendPhotoName(a);
...
function challs_flash_onCompleteAll(a){ //上传文件列表全部上传完毕事件,参数 a 数值类型，返回上传失败的数量
    //document.getElementById('show').innerHTML+='<font color="#ff0000">所有文件上传完毕，</font>上传失败'+a+'个！<br />';on
	alert("所有图片上传完毕，上传失败"+a+"个！");
